...
namespace vietlabs.fr2
{
    internal partial class FR2_Cache
    {
        ...
        private static void Check()
        {
            ...
            // This prevents constant refreshing from continuous asset changes
            if (FR2_SettingExt.isAutoRefreshEnabled && !Api._isFR2WindowFocused && Api.ready)
            {
                Api._needsRefreshOnFocus = true;
                Api.HasChanged = true;
                EditorApplication.update -= Check;
                return;
            }
        ...
        internal void IncrementalRefresh()
        {
            ...
            // With auto-refresh enabled, only refresh when FR2 window gains focus
            // This prevents constant refreshing from continuous asset changes
            if (FR2_SettingExt.isAutoRefreshEnabled && !_isFR2WindowFocused && ready)
            {
            ...
            if (AssetMap == null)
            {
                // Cache not yet initialized, trigger initialization
                Check4Changes(false);
                if (AssetMap == null)
                {
        ...
        internal void Check4Work(bool force)
        {
            if (workCount == 0)
            {
                Check4Usage();
            ...
            // With auto-refresh enabled, only process work when FR2 window gains focus
            // This prevents constant processing from continuous asset changes
            if (FR2_SettingExt.isAutoRefreshEnabled && !_isFR2WindowFocused && !force && ready)
            {
                _needsRefreshOnFocus = true;
                HasChanged = true;
                return;
